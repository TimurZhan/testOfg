...
import ru.stqa.pft.testOfg.model.MailMessage;
...
public class JamesHelper {
  ...
  public void createUser(String name, String password) {
    ...
    write("adduser " + name + " " + password); //Дается команда для добавления юзера
  ...
  private void initTelnetSession() {
    ...
    try {
      ...
      out = new PrintStream(telnet.getOutputStream()); //Перехватывается выходной поток. Нужен для передачи команд Telnetу
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public void drainEmail(String name, String password) throws MessagingException {
    Folder inbox = openInbox(name, password);
  ...
  public List<MailMessage> waitForMail(String users, String password, long timeout) throws MessagingException {
    long now = System.currentTimeMillis();
    while (System.currentTimeMillis() < now + timeout) {
      List<MailMessage> allMail = getAllMail(users, password);
      if (allMail.size() > 0) {
        return allMail;
      }
      try {
  ...
  //Этот меттод извлекает сообщение из почтового ящика и превращает его в модельный объект типа MailMessage.
  public List<MailMessage> getAllMail(String users, String password) throws MessagingException {
    Folder inbox = openInbox(users, password); //Тут почтовый ящик открывается
